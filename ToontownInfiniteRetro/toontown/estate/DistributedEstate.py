...
from toontown.building.DistributedElevator import *
from toontown.building.DistributedElevatorAI import *
import math
...
class DistributedEstate(DistributedObject.DistributedObject):
    ...
    def load(self):
        ...
        print "bobobobobobobo"

        elevatorModel = loader.loadModel('phase_12/models/bossbotHQ/BB_Elevator')
        leftDoor = elevatorModel.find('**/left-door')
        if leftDoor.isEmpty():
            leftDoor = elevatorModel.find('**/left_door')
        rightDoor = elevatorModel.find('**/right-door')
        if rightDoor.isEmpty():
            rightDoor = elevatorModel.find('**/right_door')
        rightDoor.reparentTo(render)
        leftDoor.reparentTo(render)
        rightDoor.setPos(-111.6,-108.5626,0)
        leftDoor.setPos(-73.6,-108.5626,0)
        rightDoor.setScale(0.9)
        leftDoor.setScale(0.9)
